    ...
    <div class="page-wrap">
      ...
      <main aria-labelledby="homepage-title">
        ...
        <section id="tools" class="page-section"><h2>{{ t('Tools', '工具') }}</h2><div class="tools-grid">
          ...
          <article><h3>{{ t('Japan tax calculator', '日本税务计算器') }}</h3><span class="badge">{{ t('Try it', '可试用') }}</span><p>{{ t('2025 salary income / 2026 resident tax estimates. Enter confirmed deductions; limited scenarios only.', '2025 工资收入／2026 住民税概算。扣除额需自行确认，仅适用限定场景。') }}</p><a class="text-action" :href="zh ? '/zh/tools/japan-tax.html' : '/tools/japan-tax.html'">{{ t('Try the tax calculator', '试用税务计算器') }} ↗</a></article>
        </div></section>
